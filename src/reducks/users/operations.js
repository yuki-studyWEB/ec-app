...
export const resetProductsInCart = (products) => {
    return async (dispatch) => {
        dispatch(resetProductsInCartAction(products))
    }
} // logout時storeのカートの中身を全て消す。(初期時リッスンでDBから取得する処理があるため)

export const listenAuthState = () => {
    return async (dispatch) => {
        return auth.onAuthStateChanged((user) => {
            if (user) {
                //auth.onAuthStateChanged Firebaseの認証状態チェック
                const uid = user.uid
...
export const addUserBaseData = (newUserData, password) => {
    return async (dispatch, getState) => {
        ...
        if (
            ...
            newUserData.lastKana !== '' ||
            newUserData.firstKana !== '' ||
            newUserData.postCodeH !== '' ||
            newUserData.postCodeF !== '' ||
            newUserData.address1 !== '' ||
            newUserData.address2 !== '' ||
            newUserData.address3 !== '' ||
            password !== ''
        ) {
            auth.signInWithEmailAndPassword(email, password)
                .then((result) => {
